<?php
        ...
        while($row=mysqli_fetch_assoc($result)){
            ...
            $response[0] = array("response"=>"$count");  
            $response[$count] = array("id"=>$row["id"],"u_id"=>$row["u_id"],"postimage"=>$row["postimage"],"des"=>$row["des"],"date"=>$row["date"],"likes"=>$row["likes"],"commentnumber"=>$row["comments"],"offer"=>$row["offer"],"name"=>ucwords(strtolower($row["name"])),"location"=>ucwords(strtolower($row["location"])),"role"=>$row["role"], "comments"=>$comments, "isLiked"=>$row["isLiked"]);
        }
