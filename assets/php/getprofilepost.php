<?php
        ...
        $sql_query =  "SELECT posts_table.id,posts_table.postimage,posts_table.des,posts_table.likes,posts_table.comments, posts_table.offer,posts_table.date,posts_table.u_id,profile_table.name,profile_table.location,profile_table.role FROM `posts_table` LEFT JOIN `profile_table` ON posts_table.u_id = profile_table.id WHERE `profile_table`.`id`= $id ";//SQL command
        $response = array();
        $data = array();
        ...
        while($row=mysqli_fetch_assoc($result)){
            ...
            $response[$count] = array("id"=>$row["id"],"u_id"=>$row["u_id"],"postimage"=>$row["postimage"],"des"=>$row["des"],"date"=>$row["date"],"likes"=>$row["likes"],"comments"=>$row["comments"],"offer"=>$row["offer"],"name"=>$row["name"],"location"=>$row["location"],"role"=>$row["role"], "comments"=>$comments);
        }
